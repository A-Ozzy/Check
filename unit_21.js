...
function t3(e) {
	let n = e.target.className.split("").slice(6,7).join();
	document.querySelector('.out-3').innerHTML = n;
...
// ваше событие здесь!!!
document.querySelector('.div-4').ontouchend = () => {
...
let userWidth = document.querySelector('.div-10').clientWidth++;
function t10() {
	userWidth++;
	document.querySelector('.div-10').style.width = userWidth + "px";
}
...
function t11(e) {
	let myObj = e.touches;
	let radiusX = myObj[0].radiusX;
	let radiusY = myObj[0].radiusY;
	document.querySelector('.out-11').innerHTML = `radiusX - ${radiusX} radiusY - ${radiusX}`
}
...
let activeImg = document.querySelector('.active-img').getAttribute('src');
...
function nextFunction() {
	...
	for (let i = 0; i < images.length; i++) {
		if(count == i){
			...
			let activeImg = document.querySelector('.active-img').getAttribute('src');
			document.querySelector('.div-12-max img').setAttribute('src', activeImg);
		}
